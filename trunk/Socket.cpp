...
void CommunicatingSocket::sendString(const std::string & buffer ) 
    throw(SocketException) {
  CommunicatingSocket::send(buffer.data(), buffer.length());
}
...
void CommunicatingSocket::keepAlive() throw(SocketException) {
  int keepalive = 1;
  if (setsockopt(sockDesc, SOL_SOCKET, SO_KEEPALIVE, (void*)&keepalive, sizeof(keepalive)) < 0) {
    throw SocketException("Fail to enable keep alive connection", true);
  }
}
// TCPSocket Code
